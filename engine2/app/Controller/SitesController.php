...
class SitesController extends AppController {
	...
	public function dealProductList(){
		if($this->request->is('post')){
			$postData = $this->request->input('json_decode',true);
			$this->Product->tablePrefix = $postData['tablePrefix'];
			$cond = "status = 'active' AND options > 0";
			$order = array('created'=> 'DESC');
			$page = 1;
	...
	public function singleProduct(){
		...
		if($this->request->is('post')){			
			...
			$datas = $this->Product->find(
				...
				array(
						'fields'=>array(
							'Product.id','Product.title','product_code','Product.slug','Product.description','Product.short_description','Product.price','Product.sale_price','Product.options','(Product.purchased - (Product.sold + Product.demage)) as quantity'
						),
						'contain' 	=>array(
			...
			$productAttribute = $this->Attribute->find(
				...
				array(
					'recursive'=>-1,
					'fields'=>array('id','title','useCombination'),
			...
			if(count($datas)>0){
				$response = $datas;
				$response['Attribute'] = $productAttribute;
				
				if(empty($datas['ProductAttribute'])){
					$stockData = $this->ProductStock->find(
						'first',
						array(
							'recursive' => -1,
							'fields' => array('id','quantity'),
							'conditions' => array('product_id' => $datas['Product']['id'])
						)
					);
					if(!empty($stockData)){
						$response['ProductStock']['quantity']= $stockData['ProductStock']['quantity'];
						$response['ProductStock']['id']= $stockData['ProductStock']['id'];
					}
	...
	//get product stock by product id and attribute-value ids
	public function getProductStock(){
		$response = array();
		if($this->request->is('post')){
			$postData = $this->request->input('json_decode',true);
			$data = $this->ProductStock->find(
				'first',
				array(
					'recursive' => -1,
					'fields' => array('id','quantity'),
					'conditions' => array(
						'attributeValues' => trim($postData['attrValues']),
			...
			$stockData = array();
			if(!empty($data)){
				if($data['ProductStock']['quantity'] > 0){
					$response['status'] = 'success';
					$stockData['id'] = $data['ProductStock']['id'];
					$stockData['stock'] = $data['ProductStock']['quantity'];
					$response['message'] = $stockData;
				}else{
				...
				if($data2['ProductStock']['quantity'] > 0){
					$response['status'] = 'success';
					$stockData['id'] = $data2['ProductStock']['id'];
					$stockData['stock'] = $data2['ProductStock']['quantity'];
					$response['message'] = $stockData;
				}else{
					$response['status'] = 'error';
		...
		$this->set(
			array(
				'_serialize',
				'data' => array('ecommerceProductStock'=>$response),
				'_jsonp' => true
			)
		);
		$this->render('data_layout');
	...
	public function productListByFeaturedCategory(){
		if($this->request->is('post')){
			...
			$categoryData = $this->getCategoryIds($postData['slug'],$isLast,$postData['parentCatSlug']);
			...
			if(count($categoryObj)>0){
					...
					$minMaxPrice = $this->Product->find(
		    			...
		    			array(
		    				...
		    				'fields'=>array('MIN(sale_price) as minPrice','MAX(sale_price) as maxPrice'),
		    				'conditions' => array(
		    					$cond,
		    					'ProductCategory.category_id' => $idds,
		    				)
					...
					$data = $this->Product->find(
						...
						array(
							...
							'contain'=>array(
								'ProductBrand'=>array('fields'=>array('brand_id'),'limit'=>'1'),
								'ProductImage'=>array('order' => array('order' => 'ASC'),'limit'=>'1')
								),
							'conditions' => array(
								$cond,
							...
							'fields' 		=> array('id','title','slug','price','sale_price','options','unit','quantity','options','status'),
							'group'=>'Product.id',
							'order' 	=> $order,
							'limit'		 => $limit,
							'page'		 => $page
					...
					if($data){
						...
						foreach($data as $key=>$val){
							...
							if(!empty($val['ProductBrand'][0]['brand_id'])){
								if(!empty($brandList[$val['ProductBrand'][0]['brand_id']])){
									$productList['Product'][$j]['brandName']= $brandList[$val['ProductBrand'][0]['brand_id']];
								}	
							}
						...
						if(count($productList['Product']) > 0){
							$productList['Product'] = array_chunk($productList['Product'],16);
						}
					}
				}	
				unset($categoryData['idds']);
				$categoryData['productMinMaxPrice'] = $minMaxPrice[0];
				//pr($productList);	
				$this->set(
					array(
						'_serialize',
						'data' => array('ecommerceProductList'=>$productList,'ecommerceCategory'=>$categoryData,'noOfProducts'=>$noOfProducts),
						'_jsonp' => true
					)
				);
		}else{
	...
	private function getCategoryIds($slug,$isLast = null,$parent_slug = null){
		$data = $this->Category->find(
			'threaded',
			array(
				'fields' => array('id','parent_id','title','slug'),
			...
			if(!empty($data[0]['Category']['parent_id'])){
				$newData = $this->Category->find(
					...
					array(
						...
						'order'  => array('order' => 'ASC'),
						'recursive' => -1,
						'conditions' => array('Category.slug'=>$parent_slug)
					)
				);
			}else{
				$newData = $data;
	...
	public function getProductsByAttrFilter(){
		if($this->request->is('post')){
			...
			if(!empty($productFilter)){
				//pr($conditionData);
				$arr = $this->getFilterVars($productFilter);
				$page = isset($arr['page'])?$arr['page']:$page;
			}
			//===============================
			 
			 
			$categoryIdsArr = $this->getCategoryIds($postData['slug']);
			$categoryIds = $categoryIdsArr['idds'];
			$productIdArr = $this->ProductCategory->find(
					'list',
					array(
						'recursive'=>-1,
						'fields'=>array('product_id'),
						'conditions'=>array('category_id'=>$categoryIds)
					)
				);
			...
			$productIds=array_unique(array_values($productIdArr));
			
			if(isset($arr['price'])){
				$minPrice = $arr['price']['minValue'];
				$maxPrice = $arr['price']['maxValue'];
				$price = " Product.sale_price BETWEEN $minPrice AND $maxPrice";
				$condition = array(
				...
				$condition = array('Product.status'=>'active','Product.id'=>$productIds);
			}
			
			if(isset($arr['containable'])){
				$condition['containable'] = $arr['containable'];
			}
			$productList=array();
			$noOfProducts=0;
			...
			if(count($categoryObj)>0){
				...
				$cond = $condition;
				$joins['joins'] = array();
				if(!empty($condition['containable'])){
					//$containable = $condition['containable'];
					$cond['ProductAttribute.attribute_id'] = $condition['containable']['attribute'];
					$cond['ProductAttributeValue.attribute_value_id'] = $condition['containable']['attributeValue'];
					unset($cond['containable']);
	...
	//for top filtering
	private function getFilterVars($conditionData){
		$arr=[];
		if(isset($conditionData['pageNo'])){
			if($conditionData['pageNo']!==''){
				$page =$conditionData['pageNo'];
				$arr['page']=$page;
			}
		}
		...
		if(isset($conditionData['limit'])){
			if($conditionData['limit']!==''){
				$limit=$conditionData['limit'];
				$arr['limit']=$limit;
			}
		}
	
		if(isset($conditionData['price'])){
			if($conditionData['price']!==''){
				$order=array('price'=>$conditionData['price']);
				$arr['order']=$order;
			}
		}
		...
		$request_attr_val_id = '';
		//attr filtes
		if(!empty($conditionData['filterRulesQuery'])){
			foreach(($conditionData['filterRulesQuery']) as $key=>$value){
					$request_attr_id[]= $key;
					foreach($value as $k=>$v){
						$request_attr_val_id[]= $v;
					}
				}
			$arr['containable']['attribute'] = $request_attr_id;
			$arr['containable']['attributeValue'] = $request_attr_val_id;
			
			 
		}
		//price between
		if(!empty($conditionData['filterByPrice'])){
	
			$arr['price'] = $conditionData['filterByPrice'];
		}
		if(!empty($conditionData['filterByTeam'])){
			$arr['containable']['team'] = $conditionData['filterByTeam'];
		}
		return $arr;
	}
	public function getAttributes(){
		...
		if($this->request->is('post')){
			...
			$this->Attribute->tablePrefix = $postData['tablePrefix'];
			$this->Attribute->AttributeValue->tablePrefix = $postData['tablePrefix'];
			$categoryIds = $this->getCategoryIds($postData['slug']);
	...
	public function stockAvailibility(){
		...
		if($this->request->is('post')){
			$post_data = $this->request->input('json_decode',true);
		 	$stockIds = array_keys($post_data['stockIds']);
			$data = $this->ProductStock->find(
				'list',
				array(
					'fields'=>array('id','quantity'),
					'conditions' => array('id' => $stockIds)
				)
			);
	...
	*/
	...
	/* private function checkAvailableProductInstock2($data){
	...
	private function checkAvailableProductInstock($data){
		...
		foreach($data as $datum){
			if(in_array($datum['stockId'],$stockIds)){
				$productQuantity[$datum['stockId']] = ($productQuantity[$datum['stockId']] + $datum['productQuantity']);
			}else{
				$productQuantity[$datum['stockId']] = $datum['productQuantity'];
				$stockIds[] = $datum['stockId'];
				$orderedProduct[$datum['stockId']]['title'] = $datum['productTitle'];
				if(!empty($datum['attributes'])){
					$orderedProduct[$datum['stockId']]['attrVal'] =  $datum['attributes'];
				}
			}
		}
		$notAvailableProduct = array();
		...
		foreach ($productQuantity as $key => $value){
			if( $value > $data[$key]){
				$notAvailableProduct[$key] = $orderedProduct[$key];
				$notAvailableProduct[$key]['quantity'] = $data[$key];
